...
export default function PersonalMessagePopup() {
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      if (!adminMessage) {
        // Get any admin user to send message to
        ...
        if (!adminRoles || adminRoles.length === 0) {
          toast({ title: 'Error', description: 'No admin available to message', variant: 'destructive' });
          return;
        }

        const { error } = await supabase.from('personal_messages').insert({
          from_user_id: userId,
          to_user_id: adminRoles[0].user_id,
            ...
            <ScrollArea className="h-[300px] p-4">
              {messages.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-xs mt-1">Send a message to admin below</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {messages.map((msg, idx) => {
                    ...
                    if (!isFromAdmin && !isMyReply) return null;
